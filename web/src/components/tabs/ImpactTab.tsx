...
export const ImpactTab = () => {
  ...
  const data = {
    confidence,
    daysLeft,
    cap: coachData?.suggestedCap ?? 15000,
    nudgeLogged: true,
    history: [
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
        <GlassCard padding="sm" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 800, fontSize: '28px', color: '#a855f7' }}>{confCount}%</div>
            <div style={{ width: '24px', height: '24px' }}>
              <svg viewBox="0 0 24 24" style={{ transform: 'rotate(-90deg)' }}>
                ...
                <motion.circle cx="12" cy="12" r="10" fill="none" stroke="#a855f7" strokeWidth="3" strokeLinecap="round" strokeDasharray="62.8" initial={{ strokeDashoffset: 62.8 }} animate={{ strokeDashoffset: 62.8 - (62.8 * data.confidence / 100) }} transition={{ duration: 1.2 }} />
              </svg>
            </div>
          </div>
          <div style={{ fontFamily: 'Outfit, sans-serif', fontSize: '12px', color: 'rgba(255,255,255,0.5)' }}>Confidence</div>
        </GlassCard>

        <GlassCard padding="sm" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          <div style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 800, fontSize: '28px', color: colorDays }}>{data.daysLeft}</div>
          <div style={{ fontFamily: 'Outfit, sans-serif', fontSize: '12px', color: 'rgba(255,255,255,0.5)' }}>Days Remaining</div>
        </GlassCard>
        ...
        <GlassCard padding="sm" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          ...
          <div style={{ fontFamily: 'Outfit, sans-serif', fontSize: '12px', color: 'rgba(255,255,255,0.5)' }}>Suggested Cap</div>
        </GlassCard>
        ...
        <GlassCard padding="sm" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          ...
          <div style={{ fontFamily: 'Outfit, sans-serif', fontSize: '12px', color: 'rgba(255,255,255,0.5)' }}>Nudge Status</div>
        </GlassCard>
      </div>
          ...
          {data.achievements.map((badge, i) => (
            <div key={badge.id} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', minWidth: '60px' }}>
              <motion.div
                ...
                animate={badge.isNew ? { scale: [1, 1.4, 1], filter: 'drop-shadow(0 0 16px rgba(250,204,21,0.8))' } : { scale: 1 }}
                  ...
                  <div key={q.name}>
                    ...
                    <div style={{ width: '100%', height: '4px', background: 'rgba(255,255,255,0.05)', borderRadius: '99px', overflow: 'hidden' }}>
                      <motion.div 
                        ...
                        style={{ height: '100%', background: 'rgba(255,255,255,0.3)', borderRadius: '99px' }}
                      />
                    </div>
                  </div>
                ))}
