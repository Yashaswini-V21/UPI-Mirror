...
export const CoachTab: React.FC<CoachTabProps> = ({ loading: loadingProp }) => {
  ...
  // ── Hooks — always called unconditionally before any early returns ────────
  const { displayedText, isTyping } = useTypewriter(data?.narrative ?? '', 14);
  const runwayCount = useCountUp(data?.runwayDays ?? 0, 1.5);
  ...
  if (loading) {
    return (
      <motion.div
        ...
        role="status" aria-label="Kira agent pipeline executing"
      >
        <motion.div variants={itemVariants}>
          <AgentPipelineVisualizer isActive={true} speed={700} />
        </motion.div>
        <motion.div variants={itemVariants}>
