...
import { ResponsiveContainer, AreaChart, XAxis, YAxis, CartesianGrid, Area, ReferenceLine, Tooltip, Legend } from 'recharts';
...
const CustomTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    <GlassCard padding="sm" style={{ background: 'rgba(3,4,10,0.96)', minWidth: '160px', pointerEvents: 'none', border: '1px solid rgba(255,255,255,0.08)' }}>
      <div style={{ fontFamily: 'Outfit, sans-serif', fontSize: '11px', color: 'rgba(255,255,255,0.5)', marginBottom: '6px' }}>Day {label}</div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
        {payload.map((item: any) => (
          <div key={item.name} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '12px' }}>
            <span style={{ display: 'flex', alignItems: 'center', gap: '6px', fontFamily: 'Outfit', fontSize: '11.5px', color: 'rgba(255,255,255,0.7)' }}>
              <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: item.stroke }} />
              {item.name}
            </span>
            <span style={{ fontFamily: 'JetBrains Mono', fontSize: '11.5px', fontWeight: 600, color: 'white' }}>
...
export const ForecastTab = ({ loading: loadingProp }: { loading?: boolean }) => {
  ...
  // Generate 3 bands of simulation data
  const chartData = useMemo(() => {
    const startBalance = budget;
    const baseBurn = burnRate;
    
    return Array.from({ length: 30 }, (_, i) => {
      const day = i + 1;
      
      // Calculate balances for three projection curves
      const baseBalance = Math.max(0, Math.round(startBalance - i * baseBurn + (Math.sin(i * 0.7) * 200)));
      const bestBalance = Math.max(0, Math.round(startBalance - i * (baseBurn * 0.8) + (Math.sin(i * 0.7) * 250)));
      const worstBalance = Math.max(0, Math.round(startBalance - i * (baseBurn * 1.2) + (Math.sin(i * 0.7) * 150)));

      return {
        day,
        "Base Case": baseBalance,
        "Best Case": bestBalance,
        "Worst Case": worstBalance,
      };
    });
  }, [burnRate, budget]);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', width: '100%' }}>
      <style>{`
        .kira-range::-webkit-slider-thumb { appearance: none; width: 18px; height: 18px; border-radius: 50%; background: #14b8a6; box-shadow: 0 0 10px rgba(20,184,166,0.7); cursor: pointer; transition: transform 0.1s; }
        .kira-range::-webkit-slider-thumb:hover { transform: scale(1.2); }
        .kira-range::-moz-range-thumb { width: 18px; height: 18px; border-radius: 50%; background: #14b8a6; border: none; cursor: pointer; box-shadow: 0 0 10px rgba(20,184,166,0.7); }
      ...
      <GlassCard padding="lg" style={{ position: 'relative' }}>
        <AnimatePresence>
          {isRefetching && (
            ...
            >
              <span aria-hidden="true" className="animate-spin" style={{ display: 'inline-block', width: 28, height: 28, border: '3px solid rgba(255,255,255,0.1)', borderTopColor: '#14b8a6', borderRadius: '50%' }} />
            </motion.div>
          )}
        </AnimatePresence>

        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          <div style={{ fontFamily: 'Outfit, sans-serif', fontSize: '16px', fontWeight: 600, color: 'white' }}>
            Multi-Scenario Projection
          </div>
          <span style={{ fontFamily: 'JetBrains Mono', fontSize: '10px', color: 'rgba(255,255,255,0.4)' }}>
            30-DAY OUTLOOK
          </span>
        </div>
        ...
        <div style={{ width: '100%', height: 320 }}>
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData} margin={{ top: 10, right: 0, left: -20, bottom: 0 }}>
              <defs>
                <linearGradient id="best-gradient" x1="0" y1="0" x2="0" y2="1">
                ...
                <linearGradient id="worst-gradient" x1="0" y1="0" x2="0" y2="1">
              ...
              <Legend verticalAlign="top" height={36} iconType="circle" wrapperStyle={{ fontFamily: 'Outfit', fontSize: '12px', color: '#fff' }} />
              ...
              <Area type="monotone" name="Best Case" dataKey="Best Case" stroke="#22c55e" strokeWidth={2} fillOpacity={1} fill="url(#best-gradient)" dot={false} animationDuration={900} animationEasing="ease-out" />
              <Area type="monotone" name="Base Case" dataKey="Base Case" stroke="#6366f1" strokeWidth={2} fillOpacity={1} fill="url(#base-gradient)" dot={false} animationDuration={900} animationEasing="ease-out" />
              <Area type="monotone" name="Worst Case" dataKey="Worst Case" stroke="#ef4444" strokeWidth={2} fillOpacity={1} fill="url(#worst-gradient)" dot={false} animationDuration={900} animationEasing="ease-out" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </GlassCard>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '28px' }}>
          ...
          <div>
            ...
            <input
              ...
              style={{
                width: '100%',
                background: `linear-gradient(to right, #14b8a6 ${((budget - 1000) / 99000) * 100}%, rgba(255,255,255,0.1) 0%)`,
              }}
            />
          </div>
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px' }}>
              <label htmlFor="cutback-slider" style={{ fontFamily: 'Outfit, sans-serif', fontSize: '14px', color: 'rgba(255,255,255,0.7)' }}>
                Cut <span style={{ color: '#14b8a6' }}>{topCategory}</span> spending
              </label>
            ...
            <input
              ...
              style={{
                width: '100%',
                background: `linear-gradient(to right, #14b8a6 ${(cutback / 80) * 100}%, rgba(255,255,255,0.1) 0%)`,
              }}
            />
          </div>
